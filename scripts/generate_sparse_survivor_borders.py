...
ALASKA_REGION = ROOT / "map_data/state_regions/zzzz_ffpa_alaska_compensation.txt"
ALASKA_BUILDINGS = ROOT / "common/history/buildings/zzzz_ffpa_alaska_compensation.txt"
ALASKA_POPS = ROOT / "common/history/pops/zzzz_ffpa_alaska_compensation.txt"
...
CONFIG = {
    "STATE_ALASKA": {
        ...
        "receiver": "M32",
        "expected_count": 1028,
        "hubs": {"city": "D82BF2", "wood": "17211C"},
        "keep": (
            "D82BF2", "2BABD1", "7CF944", "D43DAE", "F2C82C",
            "02D229", "1C913E", "3050C5", "46B363",
...
def state_region_block(directory: Path, state: str) -> str:
    anchor = re.compile(rf"^[ \t]*{state}\s*=\s*\{{", re.MULTILINE)
    next_state = re.compile(r"^[ \t]*STATE_[A-Za-z0-9_-]+\s*=\s*\{", re.MULTILINE)
    matches = []
    for path in sorted(directory.glob("*.txt")):
        ...
        matches.append(text[match.start():match.end() + following.start()].rstrip() if following else text[match.start():].rstrip())
    assert len(matches) == 1, f"expected one {state} definition, found {len(matches)}"
    return matches[0]


def assert_alaska_compensation(firefall_root: Path) -> None:
    source = state_region_block(firefall_root / "map_data/state_regions", "STATE_ALASKA")
    compensated = state_region_block(ALASKA_REGION.parent, "STATE_ALASKA")
    compensated = compensated.replace(
        "    # Concentrate displaced Yukon subsistence capacity in the Anchorage enclave.\n",
        "",
    ).replace('farm = "x2BABD1"', 'farm = "x63763A"').replace(
        "arable_land = 140", "arable_land = 30"
    )
    assert compensated == source, "Alaska state-region override drifted from Firefall"

    levels = dict(re.findall(
        r'building\s*=\s*"([^"]+)".*?levels\s*=\s*(\d+)',
        ALASKA_BUILDINGS.read_text(encoding="utf-8-sig"),
        re.DOTALL,
    ))
    assert levels == {
        "building_government_administration": "7",
        "building_paper_mill": "10",
        "building_logging_camp": "2",
    }, "Alaska building compensation must contain final combined levels"
    assert re.search(
        r"culture\s*=\s*anglo_canadian\s+size\s*=\s*1324017\b",
        ALASKA_POPS.read_text(encoding="utf-8-sig"),
    ), "Yukon population compensation changed"
...
def render(firefall_root: Path) -> str:
    ...
    assert_alaska_compensation(firefall_root)
    ...
    assert sum(len(config["keep"]) for config in CONFIG.values()) == 74
    assert sum(map(len, remainders.values())) == 2576

    lines = [
        "# Generated by scripts/generate_sparse_survivor_borders.py.",
        "# Decentralized countries receive the wilderness; survivors keep compact settlement clusters.",
        "STATES = {",
    ]
...
def main() -> None:
    ...
    if args.check:
        ...
        print("Sparse survivor borders and Alaska compensation match Firefall: 74 of 2650 provinces retained")
        return
